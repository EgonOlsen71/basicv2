...
/**
 * Cpu is a 6502 emulation. It can be used to run actual binary 6502 code. Cpu can't be instantiated directly but can be
 * obtained from a machine instance.
 * 
 * @author EgonOlsen
 * 
 */
public class Cpu
{
  ...
  public float convertFloat(int addr)
  {
    ...
    return convertFloat(ram[addr] & 0xff, ram[addr + 5] & 0xff, ram[addr + 4] & 0xff, ram[addr + 3] & 0xff,
   ...
   *          the exponent
   * @param neg
   *          the negative flag (either 0 for positive or 0xFF for negative values)
   * @param m1
   *          the mantissa, part 1
   * @param m2
   *          the mantissa, part 2
  ...
  /**
   * Returns the number of clock ticks that has passed. Each executed command will increase this number by the
   * appropriate amount.
   * 
  ...
  /**
   * Sets a new clock tick count. This can be used to reset the counter. Setting this value has no effect on the actual
  ...
  /**
   * Causes a none maskable interrupt. This has to be done from a thread other than the actual execution thread.
   */
  ...
  /**
   * Causes a maskable interrupt. This has to be done from a thread other than the actual execution thread.
   */
  ...
  /**
   * Pauses/unpauses the cpu. This has to be done from a thread other than the actual execution thread.
   * 
   * @param paused
   *          should we pause?
   */
  ...
  /**
   * Should the execution stops once it encounters a BRK (opcode 0)? Default is true.
   * 
   * @return Should it?
   */
  ...
  /**
   * Sets if the execution should stop once it encounters a BRK (opcode 0)? Default is true.
   * 
   * @param exitOnBreak
   *          Should it?
   */
