...
public class Parser
{
  ...
  /**
   * Returns the term that represents the term in the text. The result will be a binary tree formed out of term with the
   ...
   * binary tree formed out of term with the returned term being the root element.
   * 
   * @param command
   *          the command
   * @param term
  ...
  /**
   * Handles negations by adding an additional -1* in front of the negated element.
   * 
   * @param term
   *          the term
   * @return the resulting term
  ...
  private static String handleNegations(String term)
  {
    ...
    for (int i = 0; i < term.length(); i++)
    {
      ...
      if (!inString && c == '-' && wasOp)
      {
        ...
        sb.append(term.substring(0, i)).append("(-1*").append(term.substring(i + 1, end)).append(")")
  ...
  private static Term createTerm(String term, Map<String, Term> termMap, Machine machine)
  {
    ...
    if (!mTerm.contains("(") && !mTerm.contains(")"))
    {
      for (int i = 0; i < term.length(); i++)
      {
        // Hacky fix for something like a$=+"kkk" or a$=++++++"klkk"
        // todo: Enhance to handle "dddd"+++++"dllddll" as well...
        char c = term.charAt(i);
        if (c == '+')
        {
          continue;
      ...
      // System.out.println(term+": "+t.getLeft()+" # "+t.getOperator() + " # "+t.getRight());
      if (!t.isComplete())
      {
        t.setOperator(Operator.NOP);
        t.setRight(new Constant<Integer>(0));
