...
/**
 * The Parser. This class does the actual dirty work and parses the BASIC program respectively parts of it.
 */
public class Parser
{

  /**
   * Splits the line into its parts, i.e. into its different command blocks, usually separated by a colon.
   * 
   * @param line
   *          the line to split
   * @param machine
  ...
  /**
   * Returns the command in that line part. In case that the line part is a variable assignment, it will return the LET
   * command.
   * 
   * @param linePart
   *          the line part
  ...
  /**
   * Returns the variable of that line part, i.e. the left side of an assigment if there is any.
   * 
   * @param linePart
   *          the line part
   * @param machine
   ...
   *          check if this is an actual assignment. If this is false, anything at the start will be interpreted as a
   *          variable.
   * @return the variable or null, if there is none
   */
  ...
  /**
   * Returns the array variable of that line part including the terms used for sizing it.
   * 
   * @param linePart
   *          the line part
   * @param machine
  ...
  /**
   * Replaces all strings (everything wrapped in ") with the given char. The lenght of the term will stay the same. This
   * is used to ease parsing in some cases.
   * 
   * @param term
   *          the term
   * @param toReplaceWith
   ...
   *          if true, only the left part of an assignment will be taken into account. If false, the whole string will.
   * @return the variable with its index term
   */
  public static VariableAndIndex getIndexTerm(Variable var, String linePart, Machine machine,
  ...
  /**
   * Returns a function that accesses the element of an array. Internally, array access is handled as a function call on
   * that array that returns an element.
   * 
   * @param linePart
   *          the line part
   * @param var
   ...
   */
  public static Function getArrayAccessFunction(String linePart, Variable var, Map<String, Term> termMap,
      Machine machine)
  {
  ...
  /**
   * Returns the term that represents the term in the text. The result will be a binary tree build out of terms with the
  ...
  /**
   * Returns the term that represents the term in the text minus the command at the start of it. The result will be a
  ...
  /**
   * Adds the brackets. This is used to simplify expression parsing later. Actually, it might be better to convert the
   * expression from infox to RPN and use stack to evaluate them, but then again...I can't be bothered, because this
   * works as well and it's better suitable for JITting the result.
   * 
   * @param term
   *          the term
   * @return the string
  ...
  /**
   * Checks, if a text term's brackets are correctly placed. If not, it will throw a RuntimeException.
   * 
   * @param term
   *          the term to check
   */
  ...
  /**
   * Replaces logic BASIC operators in a term by a one-char representation. The resulting term isn't valid BASIC anymore
   * but it's easier to parse.
   * 
   * @param term
   *          the term
   * @return the resulting term
  ...
  public static String replaceLogicOperators(String term)
  {
    ...
    String[] replacers =
      { "OR", Operator.getOrOperator(), "AND", Operator.getAndOperator(), "NOT", "(0" + Operator.getNotOperator() };
  ...
  /**
   * A term doesn't always represent an actual term to calculate. It can be used as a wrapper for a set of parameters as
   * well. This method returns all parameters stored in a term tree in the correct order.
   * 
   * @param term
   *          the term
  ...
  /**
   * Handles negations by adding an additional -1* in front of the negated element. SOmething similar applies to terms
   * like ++++2.
   * 
   * @param term
   *          the term
  ...
  /**
   * Adds brackets to a term based on the operator order. The resulting term makes it clear which operations belong
   * together without any knowledge about the actual operator order needed.
   * 
   * @param term
   *          the term
   * @param level
  ...
  private static int findStart(String term, int pos)
  {
    ...
    for (int i = pos - 1; i >= 0; i--)
    {
      ...
      if (brackets == 0
          && ((Operator.isOperator(c) && (c != '-' || (i > 0 && !Operator.isOperator(pc) && pc != '('))) || c == '('))
      {
        return i + 1;
      }
      if (c == ')')
  ...
  /**
   * Optimizes the term by removing redundant entries that made it into the tree during its creation.
   * 
   * @param finalTerm
   *          the final term
   * @param machine
  ...
  /**
   * Creates an atom from the part of a term. The result can be a constant, a variable, a function or another term.
   * 
   * @param part
   *          the part
   * @param termMap
  ...
  /**
   * Checks if a part of a term is a term placeholder. Placeholders are inserted at parse time. They represent parts of
   * the term that have already been evaluated to sub terms.
   * 
   * @param txt
   *          the txt
  ...
  /**
   * Sets the postfix if the function has one. This applies to DEF FN functions only.
   * 
   * @param linePart
   *          the line part
   * @param fun
  ...
  /**
   * Cleans the right part of a text from white space, but only if it's not a DATA command line.
   * 
   * @param sb
   * @return
   */
