...
public class Compactor {
     ...
     * 
     * @param threshold
     *            the mininum length of the pattern to identify.
     */
     ...
     * constants to the code that aren't finally used in the end result. This
     * method removes them.
     * 
     * @param input
     *            the native code
	...
	for (int i = 0; i < input.size(); i++) {
		...
		if (rems != null && rems.size() > 1 && rems.get(0).size() > threshold) {
		    ...
		    /*
		     * if (extCount>13 && extCount<18) { System.out.println(
		     * "TRÖT: "+(extCount-1)+
		     * "---------------------------------------------------");
		     * int h=0; for (String l:input) { System.out.println(l+
