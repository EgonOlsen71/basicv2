	...
	if (pos == pattern.size()) {
		...
		for (int i = 0; i < replacement.length; i++) {
			...
			if (pos != -1) {
				...
				int endi=replacement[i].indexOf("}");
				int posi = Integer
					.valueOf(replacement[i].substring(pos + 6, endi));
				replacement[i] = replacement[i].substring(0, pos) + " " + consts[posi]+((endi<replacement[i].length()-1)?replacement[i].substring(endi+1):"");
			    }
			}
		    }
		}
			...
			if (lineRight.startsWith(leftPart)) {
				...
				if (reg.startsWith("#")) {
				    ...
				    if (lineRight.equals(reg) || (pos != -1
					    && const2Value.containsKey(lineRight.substring(pos)) && const2Value
						    .get(lineRight.substring(pos)).floatValue() == val.floatValue())) {
					return inc(ix);
				    }
				    return resetPattern();
				} else {
