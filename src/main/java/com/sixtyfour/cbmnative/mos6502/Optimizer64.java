...
public class Optimizer64 implements Optimizer {
	...
	private static final long serialVersionUID = 1L;
	{
	    ...
	    this.add(new Pattern("FAC into REG?, REG? into FAC for CMP",
		    new String[] { "{LINE0}", "{LINE1}", "{LINE2}", "{LINE6}",
			    "{LINE7}", "{LINE17}" },
		    "LDA #<{MEM0}", "LDY #>{MEM0}", "JSR $BBA2", "LDX #<{REG0}", "LDY #>{REG0}", "JSR $BBD7",
		    "LDA #<{MEM1}", "LDY #>{MEM1}", "JSR $BBA2", "LDX #<{REG1}", "LDY #>{REG1}", "JSR $BBD7",
		    "LDA #<{REG0}", "LDY #>{REG0}", "JSR $BBA2", "LDA #<{REG1}", "LDY #>{REG1}", "JSR $BC5B"));
	    this.add(new Pattern("(int) FAC into REG?, REG? into FAC for CMP",
		    new String[] { "{LINE0}", "{LINE1}", "{LINE2}", "{LINE6}",
			    "{LINE7}", "{LINE17}" },
