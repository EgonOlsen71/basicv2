...
/**
 * The Interpreter is the basic class for parsing/compiling and executing BASIC V2 programs.
 * 
 * @author EgonOlsen
 * 
 */
public class Interpreter
{
  ...
  /**
   * Instantiates a new interpreter for a BASIC program. No interpretation/compilation will take place at this stage.
   * 
   * @param code
   *          the basic code
   */
  ...
  /**
   * Instantiates a new interpreter for a BASIC program. No interpretation/compilation will take place at this stage.
   * This constructor takes an array of code lines as input. Each code line should represent a line in the BASIC
   * program.
   * 
   * @param code
   *          the basic code
  ...
  /**
   * Parses the code. This can be called before calling the actual run method to precompile the code. It doesn't have to
   * though, because run() will call it on its own if needed.
   */
  public void parse()
  {
    ...
    parsed = true;
    Logger
        .log(machine.getCommandList().size() + " commands parsed in: " + (System.nanoTime() - start) / 1000000L + "ms");
  }


  /**
   * Starts a previously parsed BASIC program. This method is similar to run() with the exception that it requires the
   * code to be parsed already. If it isn't, it will exit with a RuntimeException. This can be useful, it you want to
   * re-run the same program multiple times without parsing it over and over again.
   */
  public void start()
  {
    stop = false;
