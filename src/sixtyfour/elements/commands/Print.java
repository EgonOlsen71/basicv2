  ...
  extends AbstractCommand
{
  ...
  public ProgramCounter execute(Memory memory)
  {
    for (int i = 0; i < parts.size(); i++)
    {
      ...
      Type type = (part.term == null || part.term.isEmpty()) ? Type.NONE : part.term.getType();
      if (del == ';' || del == ' ')
      {
      ...
      if (add.equals("\n"))
      {
        OutputChannel.println(type.equals(Type.NONE) ? "" : part.term.eval(memory).toString());
      }
      else
      {
        OutputChannel.print((type.equals(Type.NONE) ? "" : part.term.eval(memory).toString()) + add);
      }
    }
      ...
      if (!inString)
      {
            ...
            || (brackets == 0 && (c == '"' || c == ')' || c == ',' || c == ';' || (c == '$' && nc != '(') || c == '%')))
        {
          if (end || !Operator.isRealOperator(nc))
          {
            ...
            {
              // Special case: SPC(...) at the end of the line act like a ;
              line += ";";
            }
