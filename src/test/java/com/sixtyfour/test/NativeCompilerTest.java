...
public class NativeCompilerTest
{
  ...
  private static void testArrayAccess0()
  {
    System.out.println("\n\ntestArrayAccess0");
    Machine machine = new Machine();
    List<Object> objs = new ArrayList<Object>(Arrays.asList(new Float[] { 1.2f, 2.3f, 4.5f, 4.5f, 1.1f, .21f, 1.1f,
    ...
    String term =
      "A((1/183)*(B+ASC(A$+\"hello\")*2+-ASC((MID$(\"32222\",2)+A$)+A$)/2),D%)*A(0,D%+D%+COS(A(0,0)-1.2))+A(1,C*2)*B(B(ASC(\"A\")-96)*4)";
    // String term =
  ...
  public static void testLinearize()
  {
    System.out.println("\n\ntestLinearize");
    String txt =
      "(A$+\" \")+ASC(I*144+SIN(PI+ASC(CHR$(ASC((A$+\"hello\")+B$)))))+(((\"world\"+\", \"+(((A$))))))+\" universe!\"";
  ...
  private static void testStringExpression1()
  {
    ...
    machine.add(new Variable("B", 65));
    String term =
      "(A$+\" \")+(\"world\"+\", \"+A$)+\" (\"+CHR$(B+(66-B))+CHR$(B)+CHR$(124*SIN(B/2))+\"!\"+\") universe!\"+\" -- \"+A$";
    testExpr(machine, term, true);
  }
  ...
  private static void testExpression4()
  {
    ...
    term =
      "10/((-2/((2+-(32-A)^2-1+(2*A)/(8*B))/100000))^-(2*SIN(((A-X)^4+(B-Y)^3+(1+(A/4)+6*2/6-2^1+C-Z)^2)^SIN(1/2)/20)))*(12345+INT(32.67/COS(A*B)*111.2)/-ABS(A)*(1/LOG(2)*3))+SIN(34)*23/7+2/(COS(A)*132)";
    // String term="2.3/(A+B)";
    testExpr(machine, term);
  }
